...
- **No sentinel file** — `migrateFromJsonFiles()` runs every startup; already-migrated files are `.json.migrated` so they're skipped. This fixes a timing gap where `session/migrator.ts` copies legacy files into `userData` _after_ the SQLite migration already ran.
