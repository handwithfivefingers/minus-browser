...
export class PermissionStore {
  ...
  setSitePermission(origin: string, permission: PermissionType, decision: PermissionDecision) {
    appDb.run('INSERT OR REPLACE INTO permissions (origin, permission, decision) VALUES (?, ?, ?)', [
      origin,
      permission,
      decision,
    ])
  ...
  getAllSites(): string[] {
    const rows = appDb.query<{ origin: string }>('SELECT DISTINCT origin FROM permissions')
