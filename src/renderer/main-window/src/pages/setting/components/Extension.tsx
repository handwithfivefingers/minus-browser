...
export const Extension = () => {
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-5 flex flex-col gap-2">
      ...
      <div className=" flex flex-col gap-2 p-2 ">
        ...
        {adblock && (
          <div className="flex gap-2 items-center pl-4 w-full justify-between">
            <span className="text-slate-500 dark:text-slate-400 w-36 text-sm">Cosmetic Filtering</span>
            <Switch value={cosmeticFiltering} onCheck={(v) => setExtension({ ...extension, cosmeticFiltering: v })} />
          </div>
        )}

        {adblock && (
          <div className="flex flex-col gap-4 bg-slate-100 dark:bg-slate-800/50 rounded border border-slate-200 dark:border-slate-700 p-2">
            {/* Stats */}
            {stats && (
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  disabled={isLoading}
                  className="h-8 px-3 rounded-lg bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300 text-xs inline-flex items-center gap-1.5 hover:bg-slate-200 dark:hover:bg-slate-600 border border-slate-200 dark:border-slate-700 cursor-pointer disabled:opacity-50"
                >
                  <IconRefresh size={12} />
                  Force Update
                </button>
              ...
              <div className="w-full flex-col">
                ...
                {adblockAutoUpdate && (
                  <div className="bg-slate-100 dark:bg-slate-800/50 rounded flex flex-col gap-2 p-2">
                    <div className="flex gap-2 items-center">
                      <span className="text-slate-500 dark:text-slate-400 w-32 text-sm">Interval</span>
                      <select
                        value={adblockAutoUpdateInterval}
                        onChange={(e) =>
                          setExtension({ ...extension, adblockAutoUpdateInterval: Number(e.target.value) })
                        }
                        className="flex-1 px-2 py-1 text-xs rounded border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 dark:text-slate-200 focus:outline-none focus:border-slate-500"
                      >
            ...
            <div className="flex flex-col gap-2">
              <div
                role="button"
                tabIndex={0}
                className="flex items-center gap-2 text-sm text-slate-600 dark:text-slate-400 cursor-pointer select-none py-1"
              ...
              {showFilters && (
                <div className="bg-slate-100 dark:bg-slate-800/50 rounded border border-slate-200 dark:border-slate-700 flex flex-col overflow-hidden">
                  <div className="sticky top-0 z-10 bg-slate-100 dark:bg-slate-800/50 p-2 pb-1">
                    <input
                      type="text"
                      placeholder="Search filters..."
                      value={filterSearch}
                      onChange={(e) => setFilterSearch(e.target.value)}
                      className="w-full px-2 py-1.5 text-xs rounded border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 dark:text-slate-200 focus:outline-none focus:border-slate-500"
                    />
                  </div>
                  <div className="max-h-80 overflow-y-auto">
                    {sortedGroups.map(([group, items]) => {
                      ...
                      return (
                        <div key={group} className="border-b border-slate-200 dark:border-slate-700 last:border-b-0">
                          <div
                            role="button"
                            tabIndex={0}
                            className="flex items-center gap-1.5 px-3 py-2 text-xs font-medium text-slate-600 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700 cursor-pointer select-none sticky top-0 bg-slate-100 dark:bg-slate-800/50"
                          ...
                          >
                            {isExpanded ? (
                              <IconChevronDown size={16} className="text-slate-400 dark:text-slate-500 shrink-0" />
                            ) : (
                              <IconChevronRight size={16} className="text-slate-400 dark:text-slate-500 shrink-0" />
                            )}
                          ...
                          {isExpanded && (
                            <div>
                              <div className="grid grid-cols-[1fr_auto] gap-2 px-3 py-1 text-[10px] text-slate-400 dark:text-slate-500 font-medium border-b border-slate-100 dark:border-slate-700">
                                <span>Name</span>
                              ...
                              {items.map(({ key, name }) => {
                                ...
                                return (
                                  <div
                                    ...
                                    className="grid grid-cols-[1fr_auto] gap-2 px-3 py-1.5 text-xs items-center cursor-pointer hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors select-none border-b border-slate-100 dark:border-slate-700 last:border-b-0"
                                  >
                                    ...
                                    <span
                                      className={clsx(
                                        ...
                                        isDisabled
                                          ? "text-slate-400 dark:text-slate-500 bg-slate-200 dark:bg-slate-700"
            ...
            <div className="flex flex-col gap-2">
              ...
              <textarea
                ...
                className="w-full max-w-md h-24 px-3 py-2 text-xs font-mono rounded border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 dark:text-slate-200 focus:outline-none focus:border-slate-500 resize-y"
              />
              <div className="mt-2">
                <button
                  type="button"
                  onClick={handleSaveCustomFilters}
                  className="h-8 px-3 rounded-lg bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300 text-xs inline-flex items-center gap-1.5 hover:bg-slate-200 dark:hover:bg-slate-600 border border-slate-200 dark:border-slate-700 cursor-pointer"
                >
                  Save Custom Filters
                </button>
              </div>
