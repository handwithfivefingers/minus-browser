...
export const NotificationBell = () => {
  ...
  return (
    <button
      ...
      className={clsx(
        ' relative z-1 flex w-full shrink-0 cursor-pointer flex-col items-center justify-center gap-1 overflow-visible rounded-md px-0.5 py-1 text-slate-500 transition-colors hover:bg-white hover:text-indigo-500 dark:text-slate-400 dark:hover:bg-slate-800'
      )}
      title="Notifications"
    >
      {unreadCount > 0 ? <IconBellFilled size={16} /> : <IconBell size={16} />}
      <span className={clsx(styles.subMenuLabel, 'text-[10px] font-medium')}>Alerts</span>
      {unreadCount > 0 && (
