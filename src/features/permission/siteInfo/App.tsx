...
export default function SiteInfoOverlay() {
  ...
  return (
    ...
    >
      <div className="animate-slide-down w-80" style={popupStyle}>
        <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-lg dark:border-slate-700 dark:bg-slate-800">
          <div className="mb-3 flex items-center gap-2">
            <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-indigo-100 text-xs font-bold text-indigo-600 dark:bg-indigo-900/30 dark:text-indigo-400">
              {hostname.charAt(0).toUpperCase()}
            </div>
            <span className="truncate text-xs font-semibold text-slate-700 dark:text-slate-300">{hostname}</span>
            <button
              type="button"
              onClick={handleClose}
              className="ml-auto cursor-pointer border-none bg-transparent p-0.5 text-slate-400 hover:text-slate-600 dark:text-slate-500 dark:hover:text-slate-300"
            >
          ...
          <div className="mb-2 text-[10px] text-slate-400 dark:text-slate-500">Permissions</div>
              ...
              {Object.entries(permissions).map(([perm, decision]) => (
                <div key={perm} className="flex items-center justify-between text-xs">
                  ...
                  <select
                    ...
                    className="cursor-pointer rounded border border-slate-200 bg-white px-1.5 py-0.5 text-xs text-slate-700 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-300"
                  >
          ...
          <div className="mt-3 flex gap-2 border-t border-slate-100 pt-2 dark:border-slate-700">
            <button
              type="button"
              onClick={handleResetAll}
              className="cursor-pointer border-none bg-transparent text-[10px] text-red-500 hover:text-red-700 dark:text-red-400 dark:hover:text-red-300"
            >
