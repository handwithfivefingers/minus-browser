export const openUserScriptManagerDialog = (input: any[]) => {
  ...
  return new Promise((resolve) => {
    ...
    const withMeta = (
      source: string,
      meta: { name: string; runAt: string; matches: string[] },
    ) => {
      ...
      const nextBlock =
        ...
        (meta.matches.length ? meta.matches : ["*://*/*"])
    ...
    items = items.map((item) => {
      ...
      return {
        ...
        matches:
          Array.isArray(item.matches) && item.matches.length
            ? item.matches
            : parsed.matches,
    ...
    const renderList = () => {
      ...
      addBtn.innerHTML = trustPolicy.createHTML(
        icon.plus + "<span>New Script</span>",
      );
    ...
    const renderForm = () => {
      ...
      addMatch.innerHTML = trustPolicy.createHTML(
        icon.plus + "<span>Rule</span>",
      );
      ...
      (selected.matches || ["*://*/*"]).forEach(
        (rule: string, index: number) => {
          ...
          remove.onclick = () => {
            selected.matches = selected.matches.filter(
