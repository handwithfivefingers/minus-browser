...
export class Tab {
  ...
  registerTabEvents() {
    ...
    this.view.webContents.on("did-start-loading", this.didStartLoad.bind(this));
    this.view.webContents.on("did-finish-load", this.didStopLoad.bind(this));
    this.view.webContents.on("did-navigate", this.didNavigate.bind(this));
  }

  unregisterTabEvents() {
    // this.view.webContents.off("did-navigate-in-page", this.urlChanged.bind(this));
    this.view.webContents.off("did-navigate-in-page", this.updateURL.bind(this));
    this.view.webContents.off("did-navigate", this.updateURL.bind(this));
    this.view.webContents.off("did-navigate", (_event, url, _isInPlace, isMainFrame) => {
      if (!isMainFrame || !url) return;
    });
    this.view.webContents.off("page-favicon-updated", this.updateFavicon.bind(this));
    this.view.webContents.off("page-title-updated", this.pageTitleUpdate.bind(this));
    // @ts-ignore
    this.view.webContents.off("did-start-loading", this.didStartLoad.bind(this));
    // @ts-ignore
    this.view.webContents.off("did-stop-loading", this.didStopLoad.bind(this));
    // @ts-ignore
    this.view.webContents.off("page-title-updated", this.updateTitle.bind(this));
    this.view.webContents.off("did-navigate", this.didNavigate.bind(this));
    this.tabEventsRegistered = false;
  }
  ...
  private didStartLoad() {
    this.updateNavigate.bind({ isLoading: true });
  }
  private didStopLoad() {
    console.log("did Stop load");
    this.updateNavigate({ isLoading: false });
    // this.view.webContents.on("did-stop-loading", this.updateNavigate.bind(this, { isLoading: false }));
    // this.view.webContents.on("did-stop-loading", this.installPlugin.bind(this));
  }
  private didNavigate() {
  ...
  private async installCredentialFocusAssist() {
    try {
      await this.view.webContents
        ...
        .then(() => {
          console.log("installCredentialFocusAssist success");
        })
        .catch((error) => {
          console.log("installCredentialFocusAssist error:", error);
        });
    } catch (error) {}
  }
