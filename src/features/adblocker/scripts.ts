...
export const SponsorBlock = () => {
  ...
  let _sbLast = 0;
  function checkSB(ts) {
    if (ts - _sbLast >= 400) { _sbLast = ts; _doCheckSB(); }
  ...
  const QUALITY_LABELS = {
    ...
    hd1080: "1080p HD", hd720: "720p HD", large: "480p",
    medium: "360p", small: "240p",
  };
...
`);
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  function boot() {
    if (document.querySelector("#et4_tog")) return;
