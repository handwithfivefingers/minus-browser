  ...
  if (contextParams.domain !== undefined) {
    ...
    */
    ...
    if (
      ...
      filterOptions.skipDomains.some((skipDomain) => isSameOriginHost(skipDomain, contextParams.domain))
    ) {
      return false;
    }
    ...
    if (
      ...
      !filterOptions.domains.some((domain) => isSameOriginHost(domain, contextParams.domain))
    ) {
      return false;
    }
...
export { parse, matchesFilters, matches, getUrlHost, isSameOriginHost };
