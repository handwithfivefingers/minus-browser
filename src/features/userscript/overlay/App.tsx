...
const Button = ({ onClick, children, style }: { onClick: () => void; children?: React.ReactNode; style?: React.CSSProperties }) => {
  return (
    <button
      type="button"
      onClick={onClick}
...
const App = () => {
  ...
  const onSubmit = async () => {
    ...
    setItems((prev) =>
      prev.map((it) => (it.id === selected.id ? { ...it, ...formValues, id: it.id } : it)),
    );
    ...
    const updatedItems = items.map((it) =>
      it.id === selected.id ? { ...it, ...formValues, id: it.id } : it,
    );
  ...
  return (
    <div
      className="w-200 overflow-hidden rounded grid h-full"
      style={{
      ...
      <div className="px-3 flex flex-col h-full gap-2 overflow-auto">
        <FormProvider {...form}>
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="flex-1 flex flex-col gap-1 p-2 overflow-hidden">
            ...
            <div className="shrink-0 justify-end flex gap-2">
              <button
                type="button"
