...
export async function handleStorage(scriptId: string, method: string, args: any[]): Promise<any> {
  appDb.run(
  ...
  switch (method) {
    ...
    case 'GM.getValue': {
      ...
      const row = appDb.get<{ value: string }>('SELECT value FROM gm_values WHERE script_id = ? AND key = ?', [
        scriptId,
        key,
      ])
    ...
    case 'GM.setValue': {
      ...
      appDb.run('INSERT OR REPLACE INTO gm_values (script_id, key, value) VALUES (?, ?, ?)', [
        scriptId,
        key,
