...
export async function handleGetTab(): Promise<any> {
  const row = appDb.get<{ value: string }>(
    "SELECT value FROM app_state WHERE key = 'gm_tab_data'",
  );
...
export async function handleSaveTab(args: any[]): Promise<void> {
  ...
  appDb.run(
    "INSERT OR REPLACE INTO app_state (key, value) VALUES ('gm_tab_data', ?)",
...
export async function handleGetTabs(): Promise<Record<string, any>> {
  const row = appDb.get<{ value: string }>(
    "SELECT value FROM app_state WHERE key = 'gm_tabs'",
  );
  return row ? JSON.parse(row.value) : {};
}
